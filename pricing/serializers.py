...
class PanelProviderSerializer(serializers.Serializer):
    ...
    def price_from_time_com_html_nodes(_self):
    ...
    def price_from_time_com_character_a_count(_self):
        response = requests.get("https://time.com/")

        if response.status_code == requests.codes.ok:
          return response.text.count("a") / 100.0

    def price_from_openlibrary_arrays(_self):
    ...
    def __arrays_over_10_elements_count(json, count=0):
    ...
    def __values_list(json):
        return json.values() if isinstance(json, dict) else json

    def __value_is_interesting(value):
